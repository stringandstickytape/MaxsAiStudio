...
namespace AiTool3.Helpers
{
    public static class SpecialsHelper
    {
        ...
        public static async Task<AiResponse> GetReadmeResponses(Model model)
        {
            ...
            var aiService = AiServiceResolver.GetAiService(model.Provider.ServiceName, null);
            var conversation = new Conversation(DateTime.Now) { systemprompt = "Update the readme and give me the complete file.  DO NOT list your changes.", messages = new List<ConversationMessage> { new ConversationMessage { role = "user", content = userMessage } } };
