...
export const useConvStore = create<ConvState>((set, get) => {
    ...
    return {
        ...
        createConv: ({ id = `conv_${uuidv4()}`, rootMessage, selectedMessageId }) => set((state) => {
            const newConv: Conv = {
                id,
        ...
        addMessage: ({ convId, message, selectedMessageId }) => set((state) => {
        ...
        setActiveConv: ({ convId, selectedMessageId }) => set((state) => {
        ...
        deleteMessage: ({ convId, messageId }) => set((state) => {
            const conv = state.convs[convId];
            if (!conv) return state;
        ...
        deleteConv: (convId) => set((state) => {
            ...
            if (state.activeConvId === convId) {
                ...
                if (newActiveId) {
                    ...
                    newSelectedMessageId = newActiveConv.messages.length > 0
                        ? newActiveConv.messages[0].id
                        : null;
                } else {
