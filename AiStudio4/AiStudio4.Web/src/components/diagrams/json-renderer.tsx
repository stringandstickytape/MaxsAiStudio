  ...
  return (
    <Collapsible open={isOpen} onOpenChange={setIsOpen}>
      <div className="flex items-center gap-1">
        ...
        {!isOpen && (
          <span className="text-gray-500">{Array.isArray(data) ? `${data.length} items` : `${items.length} keys`}</span>
        )}
      </div>
      <CollapsibleContent>
...
export const JsonRenderer: CodeBlockRenderer = {
  ...
  Component: ({ content, className }) => {
    try {
      ...
      return <div className={`${className} text-red-500 p-4`}>Invalid JSON content</div>;
    }
  },
};
