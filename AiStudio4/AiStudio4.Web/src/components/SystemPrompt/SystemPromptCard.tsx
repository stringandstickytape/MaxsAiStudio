...
import { deleteSystemPrompt, setDefaultSystemPrompt } from '@/store/systemPromptSlice';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export function SystemPromptCard({ prompt, isDefault, onEdit, onApply }: SystemPromptCardProps) {
  ...
  const handleSetDefault = () => {
    if (!isDefault) {
      dispatch(setDefaultSystemPrompt(prompt.guid));
    }
  };
  ...
  const confirmDelete = () => {
    dispatch(deleteSystemPrompt(prompt.guid));
