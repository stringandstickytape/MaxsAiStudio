...
"""
        ...
        private async Task<(bool Success, string ErrorMessage)> ApplyChangesToFile(string filePath, List<WikiChange> changes)
        {
            try
            {
                // Get ModifyFileModernTool from service provider
                var modifyFileTool = _serviceProvider.GetService<ModifyFileModernTool>();
                if (modifyFileTool == null)
                {
                    // Fallback to manual file modification if tool not available
                    return await ApplyChangesManually(filePath, changes);
                }

                // Use ModifyFileModernTool's logic for applying changes
                var toolArguments = JsonConvert.SerializeObject(new
                {
                    path = filePath,
                ...
                var result = await modifyFileTool.ProcessAsync(toolArguments, new Dictionary<string, string>());
                var success = result.WasProcessed && !result.ResultMessage.StartsWith("Error:");
                var errorMessage = result.ResultMessage.StartsWith("Error:") ? result.ResultMessage.Substring(7) : result.ResultMessage;
                return (success, success ? null : errorMessage);
            }
            catch (Exception ex)
            {
